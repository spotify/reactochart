    ...
    initScale: function initScale(props) {
        ...
        if (shouldMeasureLabels) {
            (function () {
                ...
                var margin = _lodash2['default'].transform(_this.margin, function (result, m, key) {
                ...
                var _loop = function () {
                    ...
                    xScale = makeScale(_this.xDomains, [0, innerWidth], xType);
                    yScale = makeScale(_this.yDomains, [innerHeight, 0], yType);
                    ...
                    var labelBoxes = measureAxisLabels(xTicks, yTicks, xType, yType);
                ...
                _lodash2['default'].assign(_this, { margin: margin, innerWidth: innerWidth, innerHeight: innerHeight, xScale: xScale, yScale: yScale });
            })();
        } else {
...
function formatAxisLabel(value, type) {
    return type === 'time' ? (0, _moment2['default'])(value).format('MM-DD') : value;
}

function measureAxisLabels(xLabels, yLabels, xType, yType) {
    var xLabelEls = xLabels.map(function (l) {
        return '<text class=\'chart-axis-label chart-axis-label-x\'>' + formatAxisLabel(l, xType) + '</text>';
    });
    var yLabelEls = yLabels.map(function (l) {
        return '<text class=\'chart-axis-label chart-axis-label-y\'>' + formatAxisLabel(l, yType) + '</text>';
    });
