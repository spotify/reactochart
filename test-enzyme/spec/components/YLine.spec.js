...
describe('YLine', () => {
  ...
  it('renders a .chart-line-x element in the correct place', () => {
    let wrapper = shallow(<YLine scale={linearScale} value={linearValue} />);
    expectCorrectLinePlacement(wrapper, linearValue, linearScale);

    wrapper = shallow(<YLine scale={timeScale} value={timeValue} />);
    expectCorrectLinePlacement(wrapper, timeValue, timeScale);

    wrapper = shallow(<YLine scale={ordinalScale} value={ordinalValue} />);
    expectCorrectLinePlacement(wrapper, ordinalValue, ordinalScale);
  });

  it('renders a line with the correct width', () => {
    let wrapper = shallow(<YLine scale={linearScale} value={linearValue} width={200} />);
    expect(getLineWidth(findLine(wrapper))).to.equal(200);

    wrapper = shallow(<YLine scale={linearScale} value={linearValue} width={400} />);
    expect(getLineWidth(findLine(wrapper))).to.equal(400);
  });

  it('passes lineClassName to the line', () => {
    let wrapper = shallow(<YLine scale={linearScale} value={linearValue} lineClassName={'test-line-class'} />);
    expect(wrapper).to.have.descendants('line.test-line-class');
  });

  it('passes lineStyle to the line', () => {
    const style = {fill: 'red'};
    let wrapper = shallow(<YLine scale={linearScale} value={linearValue} lineStyle={style} />);
