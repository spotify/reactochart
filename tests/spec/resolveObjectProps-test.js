...
describe('resolveObjectProps', () => {
  ...
  const XYResolvedMissing = resolveObjectProps(XYPropTest, ['domain', 'axisType', 'missing'], ['x', 'y']);
  ...
  it('uses defaultProps normally for undefined object props', () => {
    ...
    _.keys(defaultProps).forEach(k => {
      expect(resolved.props[k]).to.be.equal(defaultProps[k]);
  ...
  it('resolves incompletely-specified object props into fully-specified objects using defaultProps', () => {
    ...
    expect(resolved.props.domain.y).to.be.equal(XYPropTest.defaultProps.domain.y);
    expect(resolved.props.axisType.x).to.be.equal(XYPropTest.defaultProps.axisType.x);
    expect(resolved.props.domain.x).to.be.equal(props.domain.x);
    expect(resolved.props.axisType.y).to.be.equal(props.axisType.y);
  });

  it('resolves single values into fully-specified objects using defaultProps', () => {
    const props = {
      domain: 0,
      axisType: "uffish"
    };
