...
describe('RangeRect', () => {
  it('renders a RangeRect with correct placement and size', () => {
    testWithScales(['linear', 'ordinal'], ({scale: xScale, testValues: xTestValues}) => {
      testWithScales(['linear', 'ordinal'], ({scale: yScale, testValues: yTestValues}) => {
        ([0, 1]).forEach((i) => {
          const el = mount(<RangeRect {...{
            xScale, yScale,
            x: xTestValues[i],
            y: yTestValues[i],
          ...
          expect(rectProps.width).to.equal(Math.abs(xEndPosition - xPosition));
          ...
          expect(rectProps.height).to.equal(Math.abs(yEndPosition - yPosition));
        });
      });
    });
  });
