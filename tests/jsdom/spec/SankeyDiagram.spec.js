...
describe('SankeyDiagram', () => {
  ...
  describe('SankeyNodeLabel', () => {
    ...
    it('uses nodeLabelText accessor prop to create label text, falls back to nodeId if nodeLabelText not provided', () => {
      const labelWithName = mount(<SankeyNodeLabel {...{
        node: basicNodeObj,
      ...
      const labelWithId = mount(<SankeyNodeLabel {...{
        node: basicNodeObj,
        nodeId: (node) => node.id
      }} />);
    ...
    it('passes nodeLabelClassName and nodeLabelStyle through to the text element', () => {
      ...
      const label = mount(<SankeyNodeLabel {...{node: basicNodeObj, nodeLabelText, nodeLabelClassName, nodeLabelStyle}} />);
      const text = label.find('text');
      expect(text).to.have.length(1);
    ...
    it('uses nodeLabelPlacement to determine the label\'s position', () => {
      ...
      const labelConditional = mount(<SankeyNodeLabel {...{...commonProps, nodeLabelPlacement: conditionalPlacement}} />);
    ...
    it('uses nodeLabelDistance to determine node label\'s distance from the node', () => {
      ...
      const labelDynamic = mount(<SankeyNodeLabel
        {...{...commonProps, nodeLabelDistance: getDistance, nodeLabelPlacement: 'below'}}
      />);
